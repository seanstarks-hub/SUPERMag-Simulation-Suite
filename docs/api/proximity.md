...
| `gamma_B` | float | `0.0` | Interface barrier parameter |
...
- **`model="thin_s"`** uses EQ-4: $F(T) = \ln(T_{c0}/T) - \text{Re}[\psi(\tfrac{1}{2} + \alpha) - \psi(\tfrac{1}{2})]$ with $\alpha = \frac{\gamma}{\gamma_B + K} \tfrac{T_{c0}}{2\pi T} + \lambda_\text{dep}$
- **`model="fominov"`** uses EQ-5: same form with $\alpha = \frac{\gamma}{\gamma_B + K + \Omega_S(T)} \tfrac{T_{c0}}{2\pi T} + \lambda_\text{dep}$ where $\Omega_S(T) = \sqrt{T/T_{c0}} \coth(\sqrt{T/T_{c0}} \cdot d_S/\xi_S)$
