...
let gl_minimize ~alpha ~beta ~kappa ~nx ~ny ~dx =
  let n = nx * ny in
  let psi_real = CArray.make double n in
  let psi_imag = CArray.make double n in
  let rc = c_gl_minimize alpha beta kappa nx ny dx
